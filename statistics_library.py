...
class Statistics:
    ...
    def prepareDataFrame(self, df, logLevel):
    ...
    def calculateChange(self, time_1, time_2, lastTimestamp, saveFolder, save_file, logLevel):
    ...
    def createSummaryStatistics(self, summary_list, timestamp, c, saveFolder, save_file, logLevel):
    ...
    def statisticsFromFiles(self, dataFolder, summaryStatList, saveFolder, logLevel):
        ...
        try:
            ...
            for count in range(0, len(rawStatsFiles) - 1):
                ...
                time_1 = self.prepareDataFrame(pd.DataFrame.from_dict(dict_train), logLevel)
                lastTimestamp = time_1['timestamp'].max()
                summary_list = self.getStatisticList(summaryStatList, time_1)


                with open(os.path.join(dataFolder, rawStatsFiles[count + 1])) as train_file:
                    dict_train = json.load(train_file)
                time_2 = self.prepareDataFrame(pd.DataFrame.from_dict(dict_train), logLevel)
                thisTimestamp = time_2['timestamp'].max()
                
                save_file = rawStatsFiles[count + 1]
                change = self.calculateChange(time_1, time_2, lastTimestamp, saveFolder, save_file, logLevel)

                py_date = thisTimestamp.to_pydatetime()
                timestamp = py_date.strftime("%Y-%m-%dT%H:%M:%S")
                data = self.createSummaryStatistics(summary_list, timestamp, change, saveFolder, save_file, logLevel)
                result += data
    ...
    def getTestStatistics(self, dataFolder, offset, logLevel):
    ...
    def getStatList(self, summaryStatList, logLevel):
    ...
    def saveStatList(self, summaryStatListUrl, summaryStatList, logLevel):
    ...
    def statisticsFromUrl(self, url, summaryStatList, saveFolder, maxInterval, logLevel):

        result = []
        try:
            response = self.getStatisticsAPI(url)

            time_2 = self.prepareDataFrame(pd.json_normalize(response), logLevel)
            thisTimestamp = time_2['timestamp'].max()
            summary_list = self.getStatisticList(summaryStatList, time_2)
            ...
            if len(items) > 1:
                ...
                if interval <= maxInterval:
                    lastTimestamp = time_1['timestamp'].max()
                    change = self.calculateChange(time_1, time_2, lastTimestamp, saveFolder, save_file_name, logLevel)
                    data = self.createSummaryStatistics(summary_list, thisTimestamp, change, saveFolder, save_file_name, logLevel)
                    result += data
                elif logLevel > 0:
    ...
    def getStatisticList(self, summaryStatList, pd):
        ...
        for a in auto:
            result += self.getUniqueValues(pd, a[1], a[2], a[3])
        return result
        
    def getUniqueValues(self, pd, index, matching, prefix):
